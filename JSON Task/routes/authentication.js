...
router.get('/checkUsernamesv/:username' , (req, res) =>{
    ...
    router.get('/checkEmailsv/:email' , (req ,res ) =>{
        if(!req.params.email){
...
router.get('/profilesv' , (req, res)=>{
       sv.findOne({_id : req.decoded.userID}).select('username email').exec((err, sv)=>{
           if(err){
               res.json({success:false ,  message:err})
           }else {
               if (!sv){
